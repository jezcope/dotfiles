...
syn keyword pentadactylCommand loadplugins lpl group gr ! run Clistkeys Clistk
    \ Clk Cmap Cm Cnoremap Cno Cunmap Cunm Ilistkeys Ilistk Ilk Imap Im Inoremap
    \ Ino Iunmap Iunm abbreviate ab addons addo ao autocmd au back ba bdelete bd
    \ bwipeout bw bunload bun tabclose tabc blistkeys blistk blk bmap bm bmark
    \ bma bmarks bnoremap bno buffer b buffers files ls tabs bunmap bunm
    \ cabbreviate ca cd chdir chd clistkeys clistk clk cmap cm cnoremap cno
    \ colorscheme colo command com completions comp contexts cookies ck
    \ cunabbreviate cuna cunmap cunm delbmarks delbm delcommand delc delgroup
    \ delg delmacros delmac delmarks delm delqmarks delqm delstyle dels dialog
    \ dia doautoall doautoa doautocmd do downloads downl dl echo ec echoerr
    \ echoe echomsg echom else el elseif elsei elif emenu em endif en fi execute
    \ exe extadd exta extdelete extde extdisable extd extenable exte extoptions
    \ exto extpreferences extp extrehash extr exttoggle extt extupdate extu
    \ feedkeys fk finish fini forward fo fw frameonly frameo hardcopy ha help h
    \ helpall helpa highlight hi history hist hs iabbreviate ia if ilistkeys
    \ ilistk ilk imap im inoremap ino iunabbreviate iuna iunmap iunm javascript
...
syn keyword pentadactylOption activate act altwildmode awim autocomplete au
    ...
    \ showtabline stal suggestengines timeoutlen tmol titlestring urlseparator
    \ urlsep us verbose vbs wildanchor wia wildcase wic wildignore wig wildmode
    \ wim wildsort wis wordseparators wsp
...
let s:toggleOptions = ["banghist", "bh", "errorbells", "eb", "exrc", "ex",
    ...
    \ "jsdebugger", "jsd", "more", "online", "private", "pornmode",
    \ "strictfocus", "sf", "timeout", "tmo", "usermode", "um", "visualbell",
    \ "vb"]
