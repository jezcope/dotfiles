...
#+BEGIN_SRC emacs-lisp
  (define-key ctl-x-map "l"
    (defhydra jc/launcher-hydra (:exit t)
      ...
      ("f" sunrise "sunrise")
      ("p" helm-package "install packages")))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key global-map (kbd "C-c F")
    (defhydra jc/find-files-hydra (:color blue)
      ("h" (helm-find-files-1 "~/") "home")
      ("d" (helm-find-files-1 (expand-file-name "./" jc/dropbox-location)) "dropbox")
      ("n" (helm-find-files-1 (expand-file-name "Notes/" jc/dropbox-location)) "notes")
      ("b" (helm-find-files-1 (expand-file-name "Notes/00Pending/Blog drafts/" jc/dropbox-location)) "blog drafts")))
#+END_SRC
