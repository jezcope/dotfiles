...
Thanks to suggestions on [[http://endlessparentheses.com/launcher-keymap-for-standalone-features.html][Endless Parentheses]] for this.

#+BEGIN_SRC emacs-lisp
  (define-prefix-command 'jc/launcher-map)
  (define-key ctl-x-map "l" 'jc/launcher-map)
  (global-set-key (kbd "s-l") 'jc/launcher-map)

  (dolist (binding '(
                     ("e" . ecb-activate)
                     ("g" . magit-status)
                     ("t" . jc/ansi-term-with-zsh)
                     ("m" . mu4e)
                     ("f" . sunrise)
                     ("i" . jc/find-inbox-file)
                     ))
    (define-key jc/launcher-map (car binding) (cdr binding)))
#+END_SRC

*** Toggle map

Also inspired by [[http://endlessparentheses.com/the-toggle-map-and-wizardry.html][Endless Parentheses]].

#+BEGIN_SRC emacs-lisp
  (define-prefix-command 'jc/toggle-map)
  (define-key ctl-x-map "t" 'jc/toggle-map)
  ...
  (dolist (binding '(
    ...
    (define-key jc/toggle-map (car binding) (cdr binding)))
#+END_SRC
...
*** Function keys
  ...
  +-------+-------+--------+------+--------+----------+------+------+------+-------+------+------+------+
  |       |Org    |Org     |Macro |Macro   |Activate  |Magit |Term  |mu4e  |Toggle |Menu  |      |      |
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "<f7>") 'jc/ansi-term-with-zsh)
