  ...
  (setq org-capture-templates
        `(("t" "Todo" entry
          ...
          ("L" "Logbook entry (date prompt)" entry
           ...
           "* %?")
          ("w" "Work logbook entry" entry
           (file+datetree jc/logbook-file-work)
           "* %?")
          ("W" "Logbook entry (date prompt)" entry
...
#+BEGIN_SRC emacs-lisp
  (setq org-log-into-drawer t
        ...
        jc/todo-keywords
        '("TODO(t) COOL(c) WARM(w) HOT(h) DOING(o!) | DONE(d!) DROPPED(x@)"
          "WAITING(a@) DELEGATED(l@) MONITOR(m) | ONHOLD(H)"
          "BACKLOG(B) READY(R) WIP(W!) | COMPLETE(D!)")
        org-todo-keywords
        ...
        '(("TODO"      . "grey")
          ...
          ("BACKLOG"   . (:background "steel blue" :foreground "white"))
          ("READY"     . (:background "dark red" :foreground "white"))
          ("WIP"       . (:background "gold" :foreground "black"))
          ("COMPLETE"  . (:background "forest green" :foreground "white"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun jc/deadline-within (seconds)
    (let ((deadline (org-entry-get (point) "DEADLINE")))
      (and deadline
           (time-less-p (current-time)
                        (apply 'encode-time (org-parse-time-string deadline)))
           (time-less-p (apply 'encode-time (org-parse-time-string deadline))
                        (time-add (current-time) seconds)))))

  (defun jc/skip-if-deadline-within (seconds)
    "Skip any task with a deadline sooner than seconds away"
    (save-restriction
      (widen)
      (let ((next-heading (save-excursion (or (outline-next-heading) (point-max)))))
        (when (not (jc/deadline-within seconds))
          next-heading))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (setq jc/org-agenda-task-cmds
        ...
        `(("p" "Personal tasks" ,jc/org-agenda-task-cmds
          ...
          ("w" . "Work tasks")
          ("ww" "Work kanban"
           ((todo "TODO|COOL|WARM|HOT|DOING|WAITING"
                  ((org-agenda-overriding-header "Overdue")
                   (org-agenda-skip-function '(or (jc/skip-if-deadline-not-passed)
                                                  (jc/skip-future-tasks-with-inheritance)))))
            (tags-todo "+TODO=\"DOING\"-delegated"
                       ((org-agenda-overriding-header "In progress")
                        (org-agenda-skip-function 'jc/skip-projects)))
            (todo "TODO|COOL|WARM|HOT|DOING|WAITING"
                  ((org-agenda-overriding-header "Due in the next 7 days")
                   (org-agenda-skip-function '(or (jc/skip-if-deadline-within (days-to-time 7))
                                                  (jc/skip-future-tasks-with-inheritance)))))
            (todo "WAITING"
                  ((org-agenda-overriding-header "Waiting for")
                   (org-agenda-skip-function '(or (jc/skip-if-deadline-passed)
